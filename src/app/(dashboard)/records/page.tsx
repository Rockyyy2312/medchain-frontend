...
export default function RecordsPage() {
    ...
    const [activeFilters, setActiveFilters] = useState<string[]>(['Radiology']);
    ...
    return (
        <div className="min-h-full bg-slate-50/50 pb-20 relative">
            <div className={`flex gap-8 p-8 max-w-[1400px] mx-auto animate-in fade-in duration-500 mt-2 transition-all ${isUploadOpen ? 'pr-8' : 'pr-8'}`}>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 bg-white rounded-xl border border-slate-200 p-1 shadow-sm">
                                <button
                                    ...
                                    className={`p-2 active:scale-95 rounded-lg transition-all ${viewMode === 'grid'
                                ...
                                <button
                                    ...
                                    className={`p-2 active:scale-95 rounded-lg transition-all ${viewMode === 'list'
                                            ? 'bg-blue-50 text-blue-600'
                                            : 'text-slate-400 hover:text-slate-900 hover:bg-slate-50'
                                        }`}
                                >
                    ...
                    <div className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-100/50 space-y-4">
                        ...
                        <div className="flex items-center gap-3 px-2 flex-wrap">
                            ...
                            {[
                                ...
                                { name: 'Cardiology', color: 'emerald' },
                                { name: 'Lab Results', color: 'orange' }
                            ].map(filter => (
                                <button
                                    key={filter.name}
                                    onClick={() => toggleFilter(filter.name)}
                                    className={`flex items-center gap-2 px-5 py-2.5 active:scale-95 font-bold text-[13px] rounded-full transition-all ${activeFilters.includes(filter.name)
                ...
                {/* Upload Flow Sidebar/Modal (Conditionally Rendered based on State) */}
                {isUploadOpen && (
                    <div className="w-[400px] bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 p-8 flex flex-col sticky top-28 h-[calc(100vh-140px)] shrink-0 animate-in slide-in-from-right-8 duration-300">
                        ...
                        <div className="flex items-center justify-between mb-8 shrink-0">
                            ...
                            <button
                                ...
                                className="p-2 -mr-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-full active:scale-95 transition-all"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        {/* Drop Zone */}
                        <div className="border-2 border-dashed border-blue-200 bg-blue-50/50 rounded-3xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-blue-50 hover:scale-[1.02] active:scale-95 transition-all mb-6 shrink-0"
                        ...
                        {/* Uploading Item */}
                        {fileVisible && (
                            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 mb-8 relative group shrink-0 animate-in fade-in zoom-in-95 duration-200">
                                <div className="flex items-start gap-4 mb-4 pr-6">
                                    ...
                                    <button
                                        ...
                                        className="absolute top-4 right-4 text-slate-400 hover:text-red-600 active:scale-95 transition-colors"
                                    >
                                ...
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <span className="text-[11px] font-extrabold text-blue-600 tracking-wider">COMPLETED</span>
                                        <span className="text-[11px] font-extrabold text-blue-600">100%</span>
                                    </div>
                                    <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                        <div className="h-full bg-blue-600 rounded-full transition-all duration-1000 w-full"></div>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="space-y-4 mb-8 flex-1 overflow-y-auto pr-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <button className="px-4 py-2 bg-blue-50 hover:bg-blue-100 active:scale-95 border border-blue-600/30 text-blue-600 text-[13px] font-bold rounded-lg shadow-sm transition-all flex items-center gap-1">
                                    Radiology <CheckCircle2 className="w-3 h-3" />
                                </button>
                                <button className="px-4 py-2 bg-white hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 active:scale-95 border border-slate-200 text-slate-600 text-[13px] font-bold rounded-lg transition-all">
                                    Lab Result
                                </button>
                                <button className="px-4 py-2 bg-white hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 active:scale-95 border border-slate-200 text-slate-600 text-[13px] font-bold rounded-lg transition-all">
                                    Prescription
                        ...
                        <button
                            ...
                            className={`w-full py-4 font-bold rounded-2xl text-[15px] transition-all shrink-0 ${fileVisible
            ...
            {!isUploadOpen && (
                <button
                    ...
                    className="fixed bottom-10 right-10 w-16 h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-xl flex items-center justify-center transition-transform hover:scale-105 active:scale-95 z-50 animate-in zoom-in duration-300"
                >
                    <div className="relative">
